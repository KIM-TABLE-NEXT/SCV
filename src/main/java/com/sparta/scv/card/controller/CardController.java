...
@RestController
@Tag(name = "Card 관리", description = "Card 관리에 관련된 API")
...
public class CardController {
    ...
    public ResponseEntity<CardStatusResponse> createCard(@RequestBody CardRequest cardRequest,
        @AuthenticationPrincipal
        UserDetailsImpl userDetails) {
        return ResponseEntity.status(HttpStatus.CREATED)
            .body(cardService.createCard(cardRequest, userDetails.getUser()));
    }

    @BoardMemberOnly
    @GetMapping("/{cardId}")
    @Operation(summary = "카드 조회", description = "카드 ID를 통해 카드를 조회한다.")
    public CardResponse getCard(@RequestBody BoardIdRequestDto boardIdRequestDto,
    ...
    public ResponseEntity<CardStatusResponse> updateCard(
        @RequestBody CardUpdateRequest cardUpdateRequest, @PathVariable Long cardId,
        @AuthenticationPrincipal
        UserDetailsImpl userDetails) {
        return ResponseEntity.status(HttpStatus.CREATED)
            .body(cardService.updateCard(cardId, cardUpdateRequest, userDetails.getUser()));
    }

    @BoardMemberOnly
    @PutMapping("/{cardId}/boardcolumns/{boardcolumnId}")
    @Operation(summary = "카드 컬럼 변경", description = "카드 ID와 컬럼 ID를 통해 카드를 이동한다.")
    public ResponseEntity<CardStatusResponse> moveCard(
        @RequestBody BoardIdRequestDto boardIdRequestDto, @PathVariable Long cardId,
        @PathVariable Long boardcolumnId, @AuthenticationPrincipal UserDetailsImpl userDetails) {
        return ResponseEntity.status(HttpStatus.CREATED)
            .body(cardService.moveCard(cardId, boardcolumnId, userDetails.getUser()));
    }

    @BoardMemberOnly
    @DeleteMapping("/{cardId}")
    @Operation(summary = "카드 삭제", description = "카드 ID를 통해 카드를 삭제한다.")
    public ResponseEntity<CardStatusResponse> deleteCard(
        @RequestBody BoardIdRequestDto boardIdRequestDto, @PathVariable Long cardId,
        @AuthenticationPrincipal
        UserDetailsImpl userDetails) {
        return ResponseEntity.status(HttpStatus.OK)
            .body(cardService.deleteCard(cardId, userDetails.getUser()));
    }
}
