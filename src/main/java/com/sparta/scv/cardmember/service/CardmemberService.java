...
public class CardmemberService {
    ...
    @Transactional
    public CardMemberStatusResponse createCardMember(CardMemberRequest cardMemberRequest,
        User user) {
        ...
        Optional<CardMember> verify = cardMemberRepository.findByCardIdAndUserId(card.getId(),
            member.getId());
        if (verify.isPresent()) {
        ...
        return new CardMemberStatusResponse(201, "CREATED", cardMember.getCard().getId(),
            cardMember.getUser().getId());
    }

    public List<CardMemberResponse> getCardMember(CardMemberIdRequest cardMemberIdRequest) {
        List<CardMember> cardMemberList = getCardMemberListByCardId(
            cardMemberIdRequest.getCardId());
        List<CardMemberResponse> cardMemberResponseList = new ArrayList<>();
        for (CardMember cardMember : cardMemberList) {
            cardMemberResponseList.add(new CardMemberResponse(cardMemberIdRequest.getCardId(),
                cardMember.getUser().getId()));
        }
    ...
    @Transactional
    public CardMemberStatusResponse deleteCardMember(CardMemberRequest cardMemberRequest,
        User user) {
        Card card = getCardById(cardMemberRequest.getCardId());
        ...
        cardMemberRepository.deleteByCardIdAndUserId(cardMemberRequest.getCardId(),
            cardMemberRequest.getMemberId());

        return new CardMemberStatusResponse(200, "OK", card.getId(),
            cardMemberRequest.getMemberId());
    }
